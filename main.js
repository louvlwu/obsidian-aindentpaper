...
class ParagraphSplitter {
     ...
     * 
     * @param {Map<Element, Array>} splitMap - key: 父容器, value: [{original, newNodes}]
     */
    _batchReplace(splitMap) {
        for (const [parent, splits] of splitMap) {
            // 构建新的子节点列表：保持原有顺序，只替换需要拆分的 <p>
            const newChildren = document.createDocumentFragment();
            const originalSet = new Set(splits.map(s => s.original));

            for (const child of Array.from(parent.childNodes)) {
                if (originalSet.has(child)) {
                    // 用拆分后的节点替换原始 <p>
                    const split = splits.find(s => s.original === child);
    ...
    _batchProcessParagraphs(paragraphs) {
        ...
        const splitMap = new Map(); // parent → [{original, newNodes}]
        ...
        for (const p of paragraphs) {
            ...
            if (!splitMap.has(parent)) {
                splitMap.set(parent, []);
            }
            splitMap.get(parent).push({ original: p, newNodes });
        }

        // 第二阶段：按父容器批量替换（减少 layout invalidation）
        if (splitMap.size > 0) {
            this._batchReplace(splitMap);
        }
    }
...
const SETTING_GROUPS = [
    {
        ...
        name: '启用段落首行缩进（含 <br> 自动拆分）',
        desc: '预览视图段落首行缩进，并自动拆分含 <br> 的段落',
        default: false,
        children: [
            { type: 'var-toggle', id: 'paragraph-indent-enabled', name: '段落首行缩进（CSS 变量开关）', desc: '控制 CSS text-indent 是否生效，同时控制段落拆分器是否执行 <br> 拆分', varName: '--paragraph-indent-enabled', default: 0 },
...
class FirstLineIndentPlugin extends Plugin {
    ...
    /**
     * 将纸质纹理类型短 key 转换为 SVG data URL 并设置 CSS 变量
     * 当选择 "none" 时，移除 aindentpaper-paper-texture-enabled 类，
     * 使 CSS 规则不生效，主题的网格背景自然显示
     * @param {string} typeKey - 纹理类型 key（kraft / fine / parchment / rough / none）
     */
    applyPaperTextureType(typeKey) {
        const textureUrl = PAPER_TEXTURES[typeKey] || PAPER_TEXTURES['kraft'];
        
        if (typeKey === 'none') {
            // 无纹理：移除启用类，让主题原始背景（如网格）显示
        ...
        this.applyVariable('--aindentpaper-paper-texture', textureUrl);
    }
